...
Citizen.CreateThread(function()
	while true do
		...
		DrawMarker(1, 1190.4223632812,-3329.8059082032,5.6322560310364-1.0001, 0, 0, 0, 0, 0, 0, 2.0, 3.0, 2.0, 13, 0, 0, 0, 0, 0, 2, 0, 0, 0, 0)
		...
		-- Launder check
		DrawMarker(1, 639.78234863282,2773.1804199218,42.025310516358-1.0001, 0, 0, 0, 0, 0, 0, 2.0, 3.0, 2.0, 13, 0, 0, 0, 0, 0, 2, 0, 0, 0, 0)
		...
		-- Meth check
		DrawMarker(1, 910.54754638672,-1065.3074951172,37.943222045898-1.0001, 0, 0, 0, 0, 0, 0, 2.0, 3.0, 2.0, 13, 0, 0, 0, 0, 0, 2, 0, 0, 0, 0)
		...
		-- Coke check
		DrawMarker(1, 387.51754760742,3584.7612304688,33.29222869873-1.0001, 0, 0, 0, 0, 0, 0, 2.0, 3.0, 2.0, 13, 0, 0, 0, 0, 0, 2, 0, 0, 0, 0)
		...
		-- Biker check
		DrawMarker(1, -64.228652954102,76.997901916504,71.616172790527-1.0001, 0, 0, 0, 0, 0, 0, 2.0, 3.0, 2.0, 13, 0, 0, 0, 0, 0, 2, 0, 0, 0, 0)
		if GetDistanceBetweenCoords(playerCoords.x, playerCoords.y, playerCoords.z, -64.228652954102,76.997901916504,71.616172790527, true) <= 5.0 then
			if IsControlJustPressed(0, 38) then
				loadBiker()
			end
		end	
	...
	if GetDistanceBetweenCoords(playerCoords.x, playerCoords.y, playerCoords.z, 996.88189697266,-3157.9697265625,-38.907146453858, true) <= 2.0 then
		-- TriggerEvent("fs_freemode:displayHelp", i18n.translate("exit_warehouse"))
			if IsControlJustPressed(0, 38) then
				...
				SetEntityCoords(GetPlayerPed(-1), -64.228652954102,76.997901916504,71.616172790527, 125.059)
				Citizen.Wait(1000)
				DoScreenFadeIn(1000)				
			end
		end
...
--UNCOMMENT TO ADD PERMISSIONS BACK (not used no translation Keep the same)
