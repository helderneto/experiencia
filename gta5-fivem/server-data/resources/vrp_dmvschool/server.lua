...
AddEventHandler("dmv:ttcharge", function()
	...
	if vRP.tryPayment({user_id,200}) then
		...
		vRPclient.notify(player,{"~r~Não tens dinheiro suficiente."})
	end
end)
...
local choice_asklc = {function(player,choice)
  vRPclient.getNearestPlayer(player,{10},function(nplayer)
    ...
    if nuser_id ~= nil then
      vRPclient.notify(player,{"A pedir a licença ..."})
      vRP.request({nplayer,"Pretendes mostrar a tua licença ?",15,function(nplayer,ok)
        if ok then
          MySQL.query("vRP/dmv_search", {id = nuser_id}, function(rows, affected)
            if #rows > 0 then
			  vRPclient.notify(player,{"Licença de condução: ~g~OK"})
			else
			  vRPclient.notify(player,{"Licença de condução: ~r~Não tem!"})
            end
          end)
        else
...
end, "Pede a licença a um jogador perto."}
